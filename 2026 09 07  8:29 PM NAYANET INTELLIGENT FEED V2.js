...
(function(){
  ...
  .nayanet-feed-v2 .blockBody{margin:0 0 18px;padding:0;border:0}
  .nayanet-feed-v2 .blockBody:before{display:none!important}
  .nayanet-feed-v2 .blockBody p{display:none}
  ...
  .nayanet-nutshell-head{display:flex;align-items:center;gap:10px;margin-bottom:9px;color:#fff;font-size:7px;font-weight:1000;letter-spacing:.18em}
  .nayanet-nutshell-head .nayanet-mini-icon{width:27px;height:27px;border-radius:9px;display:grid;place-items:center;background:#fff;color:#050507;box-shadow:0 0 16px #fff5,0 6px 13px #0008}
  .nayanet-nutshell-head svg,.nayanet-perspective-icon svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
  .nayanet-nutshell p{margin:0;color:#fff;font-size:15px;line-height:1.58;letter-spacing:-.01em}
  .nayanet-perspectives{display:grid;gap:13px}
  .nayanet-perspective-card{--inner-tone:#55b9ee;position:relative;padding:18px 19px 19px;border:1px solid color-mix(in srgb,var(--inner-tone) 72%,#fff 4%);border-radius:19px;background:linear-gradient(145deg,#121218,#07070a 78%);box-shadow:inset 0 1px #fff4,0 10px 0 #030304,0 17px 27px #000b,0 0 22px color-mix(in srgb,var(--inner-tone) 13%,transparent);overflow:hidden}
  .nayanet-perspective-card:before{content:"";position:absolute;left:0;top:14px;bottom:14px;width:2px;border-radius:3px;background:var(--inner-tone);box-shadow:0 0 12px var(--inner-tone),0 0 25px color-mix(in srgb,var(--inner-tone) 65%,transparent)}
  .nayanet-perspective-card:after{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(115deg,color-mix(in srgb,var(--inner-tone) 7%,transparent),transparent 35%)}
  .nayanet-perspective-head,.nayanet-perspective-content{position:relative;z-index:1}
  .nayanet-perspective-head{display:flex;align-items:center;gap:11px;margin-bottom:10px}
  ...
  .nayanet-perspective-head b{font-size:8px;letter-spacing:.15em;color:#fff}.nayanet-perspective-head span{display:block;color:#77717d;font-size:7px;margin-top:3px}
  .nayanet-perspective-content{color:#dcd6e0;font-size:12px;line-height:1.62}.nayanet-perspective-content.empty{color:#77717f;font-style:italic}
  .nayanet-perspective-card.lesson{background:linear-gradient(145deg,#17150c,#08080a 78%);border-color:#e8c76688;box-shadow:inset 0 1px #fff7,0 10px 0 #030304,0 17px 30px #000b,0 0 28px #e8c76618,0 0 15px #fff1}
  .nayanet-perspective-card.lesson:before{background:linear-gradient(#fff,#e8c766);box-shadow:0 0 12px #fff,0 0 25px #e8c766}.nayanet-perspective-card.lesson .nayanet-perspective-icon{color:#17120a;background:linear-gradient(145deg,#fff,#e8c766);border-color:#fff;box-shadow:0 0 20px #e8c76655,0 0 10px #fff8}.nayanet-perspective-card.lesson .nayanet-perspective-content{color:#f1e6b8}
  .nayanet-feed-v2 .blockFooter{margin-top:22px;padding-top:13px}
  ...
  [...root.querySelectorAll(':scope > .intelligentBlock')].forEach((block,i)=>{
    block.style.setProperty('--outer-tone',toneFor(i));
    const oldMap=block.querySelector('.perspectiveMap');
    const nutshellText=extractBlockText(block);
    if(oldMap){
      const wrap=document.createElement('div');wrap.className='nayanet-perspective-v2-wrap';
      wrap.innerHTML=`<div class="nayanet-nutshell"><div class="nayanet-nutshell-head"><span class="nayanet-mini-icon">${svg.nutshell}</span><span>IN A NUTSHELL</span></div><p>${nutshellText||'This intelligence event is still being distilled.'}</p></div><div class="nayanet-perspectives"></div>`;
      const grid=wrap.querySelector('.nayanet-perspectives');
      slots.forEach(([key,label,sub,tone])=>{
        const content=findExisting(block,key);const card=document.createElement('section');card.className='nayanet-perspective-card '+(key==='lesson'?'lesson':'');
        const toneMap={magenta:'#d86cff',purple:'#9a68ff',indigo:'#6675ff',sapphire:'#55b9ee',emerald:'#31c875',gold:'#e8c766'};card.style.setProperty('--inner-tone',toneMap[tone]);
        card.innerHTML=`<div class="nayanet-perspective-head"><span class="nayanet-perspective-icon">${svg[key]}</span><div><b>${label}</b><span>${sub}</span></div></div><div class="nayanet-perspective-content ${content?'':'empty'}">${content||'Perspective not yet captured.'}</div>`;grid.appendChild(card);
      });
