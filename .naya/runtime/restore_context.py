...
MANIFEST_PATH=NAYA/'naya-context-manifest.json'; BRIEFING_PATH=MEMORY/'NAYAPOWER-RUNTIME-BRIEFING.md'; FEED_PATH=NAYA/'INTELLIGENT-FEED.md'; PROJECT_PATH=NAYA/'projects'/'CURRENT-PROJECT.md'; START_PATH=ROOT/'START-HERE.md'; CHECKPOINT_DIR=NAYA/'checkpoints'; HANDOFF_DIR=NAYA/'handoffs'
sys.path.insert(0,str(MEMORY))
from memory_runtime import load_json, notes, retrieve, validate  # noqa: E402
ISO_Z_RE=re.compile(r'Z$'); SHA_RE=re.compile(r'\b[0-9a-f]{40}\b'); GENERATED_STATUS_RE=re.compile(r'^\?\? \.naya/(?:memory|runtime)/__pycache__/')
...
def repository_reality(at=None):
    ...
    parts=commit.split('|',2) if commit else []; raw_lines=status.splitlines() if status else []
    effective=[line for line in raw_lines if not GENERATED_STATUS_RE.match(line)]
...
def current_head_declarations(text):
    pattern=re.compile(r'(?im)^\s*(?:live|current|observed)\s+`?(?:main|head|git head)`?.{0,120}?\b([0-9a-f]{40})\b')
    return pattern.findall(text)

def orientation_projection(path,actual_head):
    body=read_text(path); declarations=current_head_declarations(body); matches=bool(actual_head and actual_head in declarations); mismatch=bool(actual_head and declarations and actual_head not in declarations)
    return {'path':str(path.relative_to(ROOT)),'available':bool(body),'current_head_declaration_count':len(declarations),'declared_current_heads':declarations[:20],'matches_observed_head':matches,'head_mismatch':mismatch}
...
def note_is_visible(note,at):
    ...
    return not (effective and effective>at) and not (note.get('superseded_at') and parse_time(note['superseded_at'])<=at)
...
def proof_target(project_text,state):
    ...
    value=state.get('single_next_action')
    return value if isinstance(value,str) else None

def build_restore(query='',at=None,limit=10):
    target=parse_time(at); state=load_state(); legacy=load_legacy_state(); manifest=load_manifest(); structural_errors=validate(); repo=repository_reality(target)
    orientation=orientation_snapshot(repo,state) if not target else {'historical':True}; project_text=read_text(PROJECT_PATH); memory=memory_snapshot(query,target,limit)
