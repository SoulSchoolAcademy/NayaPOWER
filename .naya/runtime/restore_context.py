...
from project_intelligence import build_project_intelligence  # noqa: E402
...
def build_restore(query='',at=None,limit=10):
    target=parse_time(at); state=load_state(); legacy=load_legacy_state(); manifest=load_manifest(); structural_errors=validate(); repo=repository_reality(target); orientation=orientation_snapshot(repo,state) if not target else {'historical':True}; project_text=read_text(PROJECT_PATH); memory=memory_snapshot(query,target,limit); project_intelligence_error=None
    try:
        project_intelligence=build_project_intelligence()
    except Exception as exc:
        project_intelligence={'schema':'naya/project-intelligence-context/v1','status':'UNKNOWN','error':f'{type(exc).__name__}: {exc}'}; project_intelligence_error=project_intelligence['error']
    reconciliation_reasons=list(orientation.get('contradictions',[]))+list(orientation.get('mismatches',[]))
    if project_intelligence_error: reconciliation_reasons.append('project intelligence reconstruction failed')
    reconciliation_required=bool(reconciliation_reasons) if not target else False; status='RECONCILIATION_REQUIRED' if reconciliation_required else ('VERIFIED' if not structural_errors and repo['available'] else 'UNKNOWN'); block=state.get('current_block'); next_action=state.get('single_next_action'); unknown=state.get('unknown',[])
    return {'schema':'naya-power-restore-context/v5','status':status,'generated_at':now().isoformat(),'mode':'RESTORE-TIME' if target else 'RESTORE-STANDARD','requested_at':target.isoformat() if target else None,'authority':manifest.get('authority_rules',{}),'current_state':state,'legacy_state_projection':legacy,'repository_reality':repo,'orientation':orientation,'memory':memory,'project_intelligence':project_intelligence,'stale_or_superseded':stale_memory(),'validation':{'passed':not structural_errors,'errors':structural_errors},'mission':state.get('mission'),'north_star':state.get('north_star'),'protected':state.get('protected',[]),'known':state.get('verified_evidence',{}).get('known',[]),'unknown':unknown,'what_changed':state.get('verified_evidence',{}).get('known',[]),'what_is_unfinished':unknown,'current_project':{'active_block':block,'status':state.get('current_block_status'),'priority':state.get('priority'),'target_state':state.get('bottleneck')},'active_block':block,'next_best_action':next_action,'proof_target':proof_target(project_text,state),'latest_handoff':orientation.get('latest_handoff'),'reconciliation':{'required':reconciliation_required,'reasons':reconciliation_reasons},'operational_state_authority':str(CONTROL_STATE_PATH.relative_to(ROOT)),'active_block_authority':str(CONTROL_BLOCK_PATH.relative_to(ROOT)),'proof_authority':str(CONTROL_PROOF_PATH.relative_to(ROOT))}
