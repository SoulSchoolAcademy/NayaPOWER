...
def execute_cycle():
    ...
    # STATE owns the canonical operational next action; keep STATE and BLOCKS transactionally coherent.
    state["single_next_action"]=successor
    state["next_actions"]=[successor]
    state["next_action_count"]=1
    state.setdefault("current_frontier",{})["next_action"]=successor
    state["current_next_action"]={
        "action":successor,
        "reason":"Successor action generated by the verified next_action + handoff cycle.",
    }
    ...
    receipt={
      ...
      "project_update":{"state_path":str(STATE.relative_to(ROOT)),"block_path":str(BLOCKS.relative_to(ROOT)),"map_path":str(MAP.relative_to(ROOT)),"state_path":str(STATE.relative_to(ROOT)),"updated_by":"next_action_handoff.py"},
