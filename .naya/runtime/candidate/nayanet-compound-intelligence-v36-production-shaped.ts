...
/* ===== V36 NON-PRODUCTION PORTABLE INTELLIGENCE BOUNDARY ===== */
const V36_PORTABLE_SCHEMA="naya/portable_authorization/v1", V36_ACTION_TYPE="INTELLIGENCE_COMMIT", V36_PERMISSION="intelligence_commit", V36_TARGET="NayaNET", V36_REPOSITORY="SoulSchoolAcademy/NayaPOWER";
function v36Canonical(v:unknown):string{if(Array.isArray(v))return "["+v.map(v36Canonical).join(",")+"]";if(v!==null&&typeof v==="object")return "{"+Object.entries(v as Record<string,unknown>).sort(([a],[b])=>a.localeCompare(b)).map(([k,x])=>JSON.stringify(k)+":"+v36Canonical(x)).join(",")+"}";return JSON.stringify(v)}
function v36Hex(h:string):Uint8Array{if(!/^[0-9a-fA-F]+$/.test(h)||h.length%2)throw new Error("PORTABLE_SIGNATURE_INVALID");return Uint8Array.from(h.match(/../g)!,x=>parseInt(x,16))}
async function v36Hash(s:string):Promise<string>{const d=new Uint8Array(await crypto.subtle.digest("SHA-256",new TextEncoder().encode(s)));return Array.from(d,x=>x.toString(16).padStart(2,"0")).join("")}
async function v36Bind(a:any):Promise<string>{return v36Hash([a.authority_id,a.decision_id,a.action_id,a.action_type,a.target,a.actor_id,a.scope,a.permission].join("|"))}
async function v36GrantFp(g:any):Promise<string>{return v36Hash(v36Canonical({authority_id:g.authority_id,principal_id:g.principal_id,purpose:g.purpose,scope:g.scope,granted_actions:[...(g.granted_actions??[])].sort(),expires_at:g.expires_at,revoked:Boolean(g.revoked)}))}
async function v36RegistryRev(g:any):Promise<string>{return v36Hash(v36Canonical([[g.authority_id,g.principal_id,g.purpose,g.scope,[...(g.granted_actions??[])].sort(),g.expires_at,Boolean(g.revoked)]]))}
async function verifyV36PortableArtifact(artifact:any,artifactHash:string,actorId:string,commitSha:string,ordinary:any):Promise<any>{
 const pub=String(Deno.env.get("NAYANET_PORTABLE_ISSUER_PUBLIC_KEY_HEX")??"").trim();if(!pub)throw new Error("PORTABLE_ISSUER_TRUST_ROOT_NOT_CONFIGURED");
 if(!artifact||artifact.schema!==V36_PORTABLE_SCHEMA||!artifact.authorization||!artifact.signature)throw new Error("PORTABLE_AUTHORIZATION_MALFORMED");
 const hash=await v36Hash(v36Canonical(artifact));if(hash!==String(artifactHash))throw new Error("PORTABLE_ARTIFACT_HASH_MISMATCH");
 const a=artifact.authorization, req=["authority_id","decision_id","action_id","action_type","target","actor_id","scope","permission","governance_state","binding_hash"];if(req.some(k=>!String(a[k]??"").trim()))throw new Error("PORTABLE_AUTHORIZATION_INCOMPLETE");
 const key=await crypto.subtle.importKey("raw",v36Hex(pub),{name:"Ed25519"},false,["verify"]);if(!await crypto.subtle.verify({name:"Ed25519"},key,v36Hex(String(artifact.signature)),new TextEncoder().encode(v36Canonical(a))))throw new Error("PORTABLE_SIGNATURE_INVALID");
 if(a.action_type!==V36_ACTION_TYPE||a.permission!==V36_PERMISSION||a.target!==V36_TARGET||a.governance_state!=="AUTHORIZED")throw new Error("PORTABLE_ACTION_BINDING_INVALID");if(String(a.repository??"")!==V36_REPOSITORY)throw new Error("REPOSITORY_MISMATCH");if(String(a.commit_sha??"")!==commitSha)throw new Error("SOURCE_COMMIT_MISMATCH");if(String(a.actor_id)!==actorId)throw new Error("ACTOR_MISMATCH");
 const issued=Date.parse(String(a.issued_at??"")),expires=Date.parse(String(a.expires_at??""));if(!Number.isFinite(issued)||!Number.isFinite(expires)||Date.now()<issued||Date.now()>=expires)throw new Error("PORTABLE_AUTHORIZATION_EXPIRED_OR_NOT_YET_VALID");
 const {data,error}=await admin.rpc("nayanet_validate_authority_grant",{p_authority_id:String(a.authority_id),p_actor_id:String(a.actor_id),p_action_type:V36_ACTION_TYPE,p_permission:V36_PERMISSION,p_target:V36_TARGET});if(error||!data)throw new Error("AUTHORITY_VALIDATION_FAILED");const g=Array.isArray(data)?data[0]:data;if(!g)throw new Error("AUTHORITY_VALIDATION_FAILED");if(String(g.principal_id??g.actor_id??"")!==actorId||Boolean(g.revoked))throw new Error("AUTHORITY_ACTOR_OR_REVOCATION_INVALID");if(String(a.authority_fingerprint??"")!==await v36GrantFp(g))throw new Error("AUTHORITY_FINGERPRINT_MISMATCH");if(String(a.registry_revision??"")!==await v36RegistryRev(g))throw new Error("REGISTRY_REVISION_MISMATCH");if(String(a.binding_hash)!==await v36Bind(a))throw new Error("BINDING_HASH_MISMATCH");for(const k of req)if(ordinary[k]!==a[k])throw new Error("AUTHORIZATION_ARTIFACT_MISMATCH");
 return {portable_authorization:structuredClone(artifact),portable_authorization_artifact_hash:hash};
}
  ...
  if (sourceEvent) {
    ...
    captureReceipt=await record(client,event,"intelligence.capture",
      ...
      [
        ...
        {type:"execution_authorization",authorization:receiptAuthorization}
      ],
