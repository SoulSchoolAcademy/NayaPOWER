...
const js=extracted.replace(/:\s*(unknown|string|any|Promise<[^>]+>)/g,"").replace(/\s+as\s+Record<string,unknown>/g,"").replace(/\s+as\s+any/g,"").replace(/\):\s*[A-Za-z0-9_<>\[\], ]+\s*\{/g,"){").replace(/\.match\(\/\.\.\/g\)\!/g,".match(/../g)/);
