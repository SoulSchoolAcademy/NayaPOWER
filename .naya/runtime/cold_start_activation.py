...
P={k:ROOT/v for k,v in {'manifest':'.naya/naya-context-manifest.json','boot':'.naya/NAYA-CONTEXT-BOOT-PROTOCOL.md','start':'SUPERBRAIN/AI-BOOT/START-HERE.md','pi_boot':'.naya/project-intelligence/00-NAYANET-COLD-NAYA-BOOT.md','protocol':'SUPERBRAIN/AI-BOOT/NAYA-POWER-ACTIVATION-PROTOCOL.md','clusters':'.naya/runtime/activation-intent-clusters.json','policy':'.naya/codex/HUMAN-CAPABILITY-AND-MASTERY-OPERATING-PROTOCOL.md','constitution':'.naya/codex/11-RUNTIME-CONSTITUTION.md','honor':'SUPERBRAIN/MASTER-NOTES/SN-20260827-NAYA-CODE-OF-HONOR.md','directive':'SUPERBRAIN/MASTER-NOTES/SN-20260827-10-OF-10-SYSTEM-OPERATING-DIRECTIVE.md','torch':'.naya/codex/CONSTITUTIONAL-AMENDMENT-CONTINUOUS-TORCH-PASS.md','no_orphan':'.naya/codex/CONSTITUTIONAL-AMENDMENT-NO-ORPHAN-EXECUTION.md','action_delivery':'.naya/NAYA-ACTION-DELIVERY-LAW.md','map':'.naya/control-plane/MAP.json','state':'.naya/control-plane/STATE.json','blocks':'.naya/control-plane/BLOCKS.json','proof':'.naya/control-plane/PROOF.json','smart_flow':'SUPERBRAIN/MASTER-NOTES/SN-20260912-NAYAPOWER-CONTINUOUS-SMART-FLOW-AND-COLD-NAYA-RESTORE.md','activity_board':'SUPERBRAIN/NAYA-ACTIVITY/00-NAYAPOWER-CURRENT-ACTIVITY-BOARD.md'}.items()}
...
def req_sequence(t,segments,l):
    ...
    for segment in segments:
        ...
        pos=next_pos
def main():
    m=js(P['manifest']); cm=js(P['clusters']); mp=js(P['map']); st=js(P['state']); bl=js(P['blocks']); pf=js(P['proof'])
    boot=text(P['boot']); start=text(P['start']); pi_boot=text(P['pi_boot']); protocol=text(P['protocol']); policy=text(P['policy']); constitution=text(P['constitution']); honor=text(P['honor']); directive=text(P['directive']); torch=text(P['torch']); no=text(P['no_orphan']); action_delivery=text(P['action_delivery']); smart=text(P['smart_flow']); board=text(P['activity_board'])
    ...
    req(boot,EXPECTED_POLICY,'context boot'); req(start,EXPECTED_POLICY,'START HERE'); req(start,'ready_to_run_execution','START HERE structured continuation'); req(start,'One-Network law','START HERE One-Network law'); req(start,BLOCK_CYCLE,'START HERE block cycle'); req(start,'.naya/project-intelligence/00-NAYANET-COLD-NAYA-BOOT.md','START HERE Project Intelligence bridge'); req(pi_boot,'## THE 14 COLD-NAYA QUESTIONS','Project Intelligence cold-Naya questions'); req(pi_boot,'PLAY TAG. YOU\'RE IT. NEVER RESTART FROM ZERO.','Project Intelligence successor law'); req(pi_boot,'CURRENT MASTER PROOF TARGET','Project Intelligence proof target');
    ...
    receipt={'schema':'naya/cold-start-activation-receipt/v10','status':'VERIFIED','scope':'repository-level cold-start modeled activation, Priority Zero Smart Flow continuity coherence, canonical current activity board discoverability, and Action-First Restore Gate presence','conversation_memory':'EMPTY','repository':m['repository'],'governance_branch':m['governance_branch'],'active_block':active['id'],'next_action':state_next,'action_first_gate':P['action_delivery'].relative_to(ROOT).as_posix(),'continuous_smart_flow':P['smart_flow'].relative_to(ROOT).as_posix(),'operational_state_authority':P['state'].relative_to(ROOT).as_posix(),'current_activity_board':P['activity_board'].relative_to(ROOT).as_posix(),'evidence':['canonical_repository','canonical_control_plane_map','canonical_control_plane_state','canonical_control_plane_block','canonical_control_plane_proof','live_HEAD_required_by_state_contract','exactly_one_active_block_next_action','state_block_next_action_coherence','legacy_state_projection_cannot_override_control_plane','legacy_state_projection_may_be_stale_without_becoming_authority','continuous_smart_flow_priority_zero_verified','canonical_current_activity_board_discoverable_in_boot_manifest','canonical_boot_and_activation_contract','canonical_code_of_honor','canonical_torch_and_no_orphan_contract','action_first_restore_gate_present','restored_next_action_is_bound_to_canonical_state_and_block'],'limitation':'Repository-level proof only; this gate proves the governed action contract is present and bound to current state. It cannot by itself prove an external LLM actually performed a real-world action; that remains a separate behavioral/runtime evidence boundary.','policy_sha256':hashlib.sha256(policy.encode()).hexdigest(),'smart_flow_sha256':hashlib.sha256(smart.encode()).hexdigest(),'activity_board_sha256':hashlib.sha256(board.encode()).hexdigest(),'action_delivery_sha256':hashlib.sha256(action_delivery.encode()).hexdigest()}
