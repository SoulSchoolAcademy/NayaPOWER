...
def main():
    ...
    boot=text(P['boot']); start=text(P['start']); protocol=text(P['protocol']); policy=text(P['policy']); constitution=text(P['constitution']); honor=text(P['honor']); directive=text(P['directive']); torch=text(P['torch']); no=text(P['no_orphan']); smart=text(P['smart_flow'])
    ...
    for path in ('.naya/control-plane/MAP.json','.naya/control-plane/STATE.json','.naya/control-plane/BLOCKS.json','.naya/control-plane/PROOF.json'):
        if path not in m.get('boot_order',[]): fail(f'canonical control-plane artifact absent from boot_order: {path}')
    ...
    req(policy,'DO NOT BUILD FOR THE MACHINE. BUILD FOR THE HUMAN.','human-outcome law'); req(policy,BLOCK_CYCLE,'block cycle'); req(torch,BLOCK_CYCLE,'Torch contract'); req(torch,'Every meaningful execution output must end with a ready-to-run Next Execution','Next Execution law')
    receipt={'schema':'naya/cold-start-activation-receipt/v7','status':'VERIFIED','scope':'repository-level cold-start modeled activation and Priority Zero Smart Flow continuity coherence','conversation_memory':'EMPTY','repository':m['repository'],'governance_branch':m['governance_branch'],'active_block':active['id'],'next_action':state_next,'continuous_smart_flow':P['smart_flow'].relative_to(ROOT).as_posix(),'operational_state_authority':P['state'].relative_to(ROOT).as_posix(),'legacy_state_role':lg['state_role'],'evidence':['canonical_repository','canonical_control_plane_map','canonical_control_plane_state','canonical_control_plane_block','canonical_control_plane_proof','live_HEAD_required_by_state_contract','exactly_one_active_block_next_action','state_block_next_action_coherence','legacy_state_projection_cannot_override_control_plane','continuous_smart_flow_priority_zero_verified','canonical_boot_and_activation_contract','canonical_code_of_honor','canonical_torch_and_no_orphan_contract'],'limitation':'Repository-level proof only; external LLM/provider behavior and interactive authenticated runtime remain separate evidence boundaries.','policy_sha256':hashlib.sha256(policy.encode()).hexdigest(),'smart_flow_sha256':hashlib.sha256(smart.encode()).hexdigest()}
