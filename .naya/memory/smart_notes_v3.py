...
EVENT_RE=re.compile(r'^SE-[0-9]{8}-[0-9]{6}-[a-z0-9-]+$'); NOTE_RE=re.compile(r'^SN-[0-9]{8}-[0-9]{6}-.+$'); VALID_STATUS={'ACTIVE','CANONICAL','HISTORICAL','SUPERSEDED','CONFLICTED','STALE'}
...
def validate_event(e,p):
    ...
    if dt:
        ...
        if hour is None:
            try: hour=Path(p).parent.name
            except Exception: hour=f'{dt:%H}'
        expected=f'{dt:%Y/%m/%d}/{int(hour):02d}/{e["event_id"]}.json'
...
def validate():
    ...
    report={'schema_version':1,'checked_at':'DERIVED','status':'GREEN' if not errors else 'RED','error_count':len(errors),'errors':errors,'canonical_event_count':len(ids)}; VALIDATION_REPORT.write_text(json.dumps(report,indent=2,ensure_ascii=False)+'\n',encoding='utf-8'); return errors
def build_index():
    rows=[]
