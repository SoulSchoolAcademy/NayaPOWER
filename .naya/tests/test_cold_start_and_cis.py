...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(["git","init","-q"],cwd=fresh,check=True); subprocess.run(["git","config","user.email","naya-test@example.invalid"],cwd=fresh,check=True); subprocess.run(["git","config","user.name","Naya Cold Start Test"],cwd=fresh,check=True); subprocess.run(["git","config","maintenance.auto","false"],cwd=fresh,check=True); subprocess.run(["git","config","gc.auto","0"],cwd=fresh,check=True); subprocess.run(["git","add",".naya","SUPERBRAIN","START-HERE.md"],cwd=fresh,check=True); subprocess.run(["git","commit","-qm","cold-start fixture"],cwd=fresh,check=True); assert_code_of_honor(fresh); cold=assert_restored(run_restore(fresh))
