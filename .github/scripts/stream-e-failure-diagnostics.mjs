...
export function classifySenderFailure(error,responseTrace,receiverResponseTrace){
  ...
  if(code==='PERSISTENCE_RECONSTRUCTION_FAILED')return {first_failure_boundary:'INDEPENDENT_PERSISTENCE',partial_state:hasLineage?'PARTIAL_STATE_OBSERVED':'UNDETERMINED',downstream_reachability:'REACHED_RECEIVER_BEFORE_INDEPENDENT_READ'};
  if(code==='PROJECTION_INDEX_RECONSTRUCTION_FAILED')return {first_failure_boundary:'INDEPENDENT_PROJECTION_INDEX',partial_state:hasLineage?'PARTIAL_STATE_OBSERVED':'UNDETERMINED',downstream_reachability:'REACHED_RECEIVER_BEFORE_INDEPENDENT_INDEX_READ'};
  if(code==='INDEPENDENT_RETRIEVAL_RECONSTRUCTION_FAILED')return {first_failure_boundary:'INDEPENDENT_RETRIEVAL',partial_state:hasLineage?'PARTIAL_STATE_OBSERVED':'UNDETERMINED',downstream_reachability:'REACHED_RECEIVER_BEFORE_RETRIEVAL_VERIFICATION'};
  if(code==='CAUSAL_LINEAGE_RECONSTRUCTION_FAILED')return {first_failure_boundary:'CAUSAL_LINEAGE',partial_state:hasLineage?'PARTIAL_STATE_OBSERVED':'UNDETERMINED',downstream_reachability:'REACHED_RECEIVER_BEFORE_CAUSAL_VERIFICATION'};
