...
if(!(new URL(page.url()).pathname==="/feed" && new URL(page.url()).searchParams.get("event_id")===eventId)) await page.waitForURL(url=>url.pathname==="/feed" && url.searchParams.get("event_id")===eventId,{waitUntil:"domcontentloaded",timeout:30000});
