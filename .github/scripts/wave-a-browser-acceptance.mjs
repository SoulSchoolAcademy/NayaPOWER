...
ap.on('response',async r=>{if(r.url().includes('nayanet_record_cognition_event'))console.log('LEDGER_TRACE_RESPONSE',r.status(),r.url(),(await r.text().catch(()=>'' )).slice(0,1200))});
          ...
          const collective=await ap.evaluate(()=>window.NayaAssistantRuntime.smartFeed({stream:'collective',limit:50}));const publishedItem=(collective?.items||[]).find(x=>x.publication_id===publicationId&&x.event_id===shareSourceEventId);if(!publishedItem)throw Error('SMART_SHARE_PUBLICATION_RUNTIME_RETRIEVAL_FAILED');
