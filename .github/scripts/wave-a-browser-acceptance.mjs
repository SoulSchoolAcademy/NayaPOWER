...
ap.on('response',async r=>{if(r.url().includes('nayanet_record_cognition_event'))console.log('LEDGER_TRACE_RESPONSE',r.status(),r.url(),(await r.text().catch(()=>'' )).slice(0,1200))});
          ...
          const shareSetupTitle=String(await ap.locator('.smart-feed-board').first().getAttribute('data-event-title')||'').trim();if(!shareSetupTitle)throw Error('SMART_SHARE_SETUP_TITLE_MISSING');const shareSetup=await ap.evaluate(async title=>window.NayaAssistantRuntime.captureSmartNote({title,content:'Authenticated runtime intelligence created solely as the source for the canonical Smart Share browser proof.'}),shareSetupTitle);if(!shareSetup)throw Error('SMART_SHARE_SETUP_CAPTURE_FAILED');const capturedSourceEventId=String(shareSetup?.event_id||shareSetup?.event?.event_id||'').trim();if(!capturedSourceEventId)throw Error('SMART_SHARE_CAPTURE_EVENT_ID_MISSING');await ap.reload({waitUntil:'networkidle'});await ap.waitForTimeout(800);const shareBoard=ap.locator('.smart-feed-board[data-event-id="'+capturedSourceEventId+'"]');await shareBoard.waitFor({state:'visible',timeout:10000});
