...
ap.on('response',async r=>{if(r.url().includes('nayanet_record_cognition_event'))console.log('LEDGER_TRACE_RESPONSE',r.status(),r.url(),(await r.text().catch(()=>'' )).slice(0,1200))});
          ...
          const collective=await ap.evaluate(()=>window.NayaAssistantRuntime.smartFeed({stream:'collective',limit:50}));const collectiveItems=Array.isArray(collective?.items)?collective.items:[];const comparisonTuple={publicationId,shareSourceEventId};console.log('SMART_SHARE_COLLECTIVE_RESPONSE_TRACE',JSON.stringify({comparisonTuple,status:collective?.status,ok:collective?.ok,stream:collective?.stream,limit:collective?.limit,itemCount:collectiveItems.length,items:collectiveItems}));const publishedItem=collectiveItems.find(x=>x.publication_id===publicationId&&x.event_id===shareSourceEventId);if(!publishedItem)throw Error('SMART_SHARE_PUBLICATION_RUNTIME_RETRIEVAL_FAILED:'+JSON.stringify({comparisonTuple,returnedItems:collectiveItems.map(x=>({id:x.id,event_id:x.event_id,source_id:x.source_id,publication_id:x.publication_id,title:x.title,source:x.source}))}));
          ...
          await ap.reload({waitUntil:'networkidle'});await ap.waitForTimeout(700);
          const postReloadCollective=await ap.evaluate(()=>window.NayaAssistantRuntime.smartFeed({stream:'collective',limit:50}));const postReloadItems=Array.isArray(postReloadCollective?.items)?postReloadCollective.items:[];const postReloadMatch=postReloadItems.find(x=>x.publication_id===publicationId&&x.event_id===shareSourceEventId);console.log('SMART_SHARE_RELOAD_HYDRATION_TRACE',JSON.stringify({comparisonTuple:{publicationId,shareSourceEventId},status:postReloadCollective?.status,ok:postReloadCollective?.ok,stream:postReloadCollective?.stream,itemCount:postReloadItems.length,exactTupleMatch:Boolean(postReloadMatch),matchedItem:postReloadMatch?{id:postReloadMatch.id,event_id:postReloadMatch.event_id,publication_id:postReloadMatch.publication_id,title:postReloadMatch.title}:null}));if(!postReloadMatch)throw Error('SMART_SHARE_RELOAD_IDENTITY_MISMATCH:'+JSON.stringify({publicationId,shareSourceEventId,returnedItems:postReloadItems.map(x=>({id:x.id,event_id:x.event_id,publication_id:x.publication_id,title:x.title}))}));
