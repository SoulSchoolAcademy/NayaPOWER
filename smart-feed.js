...
function renderCanonicalHub(items,stream){
 ...
 let cards=[...blocks.querySelectorAll('[data-real-smart-note]')]; const preservedSignature=blocks.dataset.nayaRealSignature||cards.map((card,i)=>String(i+1)+':'+String(card.querySelector('h3')?.textContent||'').trim()).join('|'); const projected=[...items].sort((a,b)=>{const ai=a?.intelligent_block?1:0,bi=b?.intelligent_block?1:0;if(ai!==bi)return bi-ai;return new Date(b?.intelligent_block?.updated_at||b?.created_at||b?.occurred_at||0)-new Date(a?.intelligent_block?.updated_at||a?.created_at||a?.occurred_at||0)});
 if(projected.length>cards.length){const template=cards[cards.length-1]||blocks.querySelector('.block');if(!template)return false;for(let i=cards.length;i<projected.length;i++){const clone=template.cloneNode(true);clone.removeAttribute('id');clone.querySelectorAll('[id]').forEach(el=>el.removeAttribute('id'));clone.dataset.realSmartNote='dynamic';clone.dataset.nayaDynamicCanonicalFeed='true';blocks.appendChild(clone)}cards=[...blocks.querySelectorAll('[data-real-smart-note]')];}
 ...
 cards.forEach((card,i)=>{
   ...
   const rawIb=item?.intelligent_block||{}; const ib=item?.metadata?.intelligent_block_v1||{identity:{schema_version:rawIb.schema_version,event_id:item?.event_id},truth:{state:rawIb.understanding_state||rawIb.content?.truth?.state,evidence_state:rawIb.content?.evidence?.evidence_state},authority:{state:rawIb.content?.authority?.state||'AUTHORIZED',scope:rawIb.content?.authority?.constraints?.join(', ')||'owner'},context:{visibility:rawIb.owner_scope||rawIb.content?.context?.visibility},life:{stage:rawIb.content?.lifecycle?.stage||rawIb.status},value:{state:rawIb.content?.value?.state},integrity:{content_hash:rawIb.content?.integrity?.content_hash}}; const identity=ib.identity||{},truthData=ib.truth||{},authority=ib.authority||{},context=ib.context||{},life=ib.lifecycle||{},value=ib.value||{},integrity=ib.integrity||{};
