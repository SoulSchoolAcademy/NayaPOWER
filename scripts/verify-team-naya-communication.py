...
from team_activity_event import emit_team_activity  # noqa: E402
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="naya-team-activity-") as tmp:
        ...
        second = emit_team_activity(
            intent="NAYA_DISCOVERY",
            actor_id="NAYA-A",
            session_id="SESSION-A",
            mission=mission,
            message="The canonical event store accepts Team Naya communication events.",
            next_action="Hand the verified discovery to NAYA-B.",
            recipients=["NAYA-B"],
        ...
        handoff = emit_team_activity(
            ...
            next_action="NAYA-B must retrieve the discovery and continue from it.",
            recipients=["NAYA-B"],
            evidence=[first["event_id"], second["event_id"]],
            events_root=root,
            index_path=index,
        )
        continuation = emit_team_activity(
            intent="NAYA_CONTINUING",
            actor_id="NAYA-B",
            session_id="SESSION-B",
            mission=mission,
            message="NAYA-B retrieved the shared handoff and is continuing the mission.",
            next_action="Verify the shared event count and replay idempotency.",
            recipients=["TEAM-NAYA"],
            evidence=[handoff["event_id"]],
            events_root=root,
            index_path=index,
        )

        replay = emit_team_activity(
            intent="NAYA_SIGNED_IN",
            actor_id="NAYA-A",
            session_id="SESSION-A",
            mission=mission,
