...
for label in sidebar: assert "name:'"+label+"'" in SHELL, f'missing canonical sidebar item: {label}'
assert SHELL.count("name:'")==10, 'canonical sidebar must contain exactly 10 destinations'
# Source contract mirrors the actual renderer: perspective keys are transformed
# into the visible NOTE headings by SmartFeedBoard at render time.
for token in ['IN A NUTSHELL','HUMAN','CHILD','GRANDMA','NAYA','MACHINE','ADAPTER LEARNING','WHAT IT MEANS','WHAT CAN I DO?',"WHAT'S IN IT FOR YOU",'TRUST','PROVENANCE','PRIVACY','RELATED INTELLIGENCE','ASK NAYA','CREATE SMART SPACE']:
    assert token in BOARD, f'missing intelligent board source contract: {token}'
...
print('canonical_shell=restored sidebar=10 canonical_board=SmartFeedBoard semantic_board_source=16 retired_sidebar=PASS')
