...
**Attack the remaining authority boundary, not the transport.** Prove grant-specific execution authority and revocation across MCP + REST/OpenAPI: authorized action succeeds; non-owner/wrong-scope action is denied; revocation blocks subsequent use; exact replay remains idempotent; fresh retrieval reconstructs the same receipt lineage. No new authority model or backend.
