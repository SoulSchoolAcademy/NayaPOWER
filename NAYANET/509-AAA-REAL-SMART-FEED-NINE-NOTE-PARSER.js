/* NayaNET 509 AAA — NINE CANONICAL SMART NOTE PARSER v1.1
...
function cleanChunk(chunk,n,title){let rest=chunk.trim();if(n<=3){const lines=rest.split(/\r?\n/);while(lines.length){const x=(lines[0]||'').trim();if(!x){lines.shift();continue}if(/^🧠\s*NAYA\s+POWER/i.test(x)){lines.shift();continue}if(n===1&&/^What\s+Is\s+Naya\s+Power\?/i.test(x)){lines.shift();continue}if(n===2&&/^What\s+Is\s+Naya\?/i.test(x)){lines.shift();continue}if(n===3&&/^What\s+Are\s+Smart\s+Notes\?/i.test(x)){lines.shift();continue}break}rest=lines.join('\n')}if(n===6){rest=rest.replace(/^Status:[^\n]*\n?/im,'').replace(/^Date:[^\n]*\n?/im,'').replace(/^Subject:[^\n]*\n?/im,'').replace(/^Smart Note Type:[^\n]*\n?/im,'').replace(/^Naya\s+Power\s*#06[^\n]*\n?/im,'')}return {number:n,title,sections:parseSections(rest)}}
function parse(){const found=[];for(const m of markers){const p=pos(m.re);if(p>=0)found.push({...m,p})}found.sort((a,b)=>a.p-b.p);const p6=found.find(x=>x.n===6)?.p;const ranges=[];for(let i=0;i<found.length;i++){const a=found[i],b=found[i+1];if(a.n===4&&p6>=0){const chunk4=source.slice(a.p,p6);const split=chunk4.search(/\n[-_]{20,}\s*\n\s*1\.\s*IN A NUTSHELL\s*\n\s*Intelligence Reports\b/i);if(split>0){ranges.push([4,a.p,a.p+split]);ranges.push([5,a.p+split,p6])}else ranges.push([4,a.p,p6])}else if(a.n!==4)ranges.push([a.n,a.p,b?b.p:source.length])}const out=[];for(const [n,a,b] of ranges){const meta=markers.find(x=>x.n===n);if(!meta)continue;out.push(cleanChunk(source.slice(a,b),n,n===5?'Intelligence Reports':meta.title))}return out.sort((a,b)=>a.number-b.number)}
...
window.Naya509NineNoteParser={boot,parse,version:'1.1'};
