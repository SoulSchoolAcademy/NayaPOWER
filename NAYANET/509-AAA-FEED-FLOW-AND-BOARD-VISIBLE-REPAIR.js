...
const NOTE_TITLES=['What Is Naya Power?','What Is Naya?','What Are Smart Notes?','Your Intelligence Today','Intelligence Reports','What Is the Intelligent Library?','Smart Lists','Smart Spaces','Smart Mail'];
...
function sidebar(){
  ...
  els.forEach(el=>{
    ...
    if(key){
      ...
      while(walker.nextNode()){const n=walker.currentNode;if(n.nodeValue.trim()===key){n.nodeValue=n.nodeValue.replace(key,rename[key]);break}}
      el.setAttribute('aria-label',rename[key]);
    }
  });
  const mail=qq('.nav button,.nav a',rail).filter(el=>/^Smart Mail\\b/i.test(text(el)));
...
function showBase(){const main=q('.main');if(!main)return;qq('.naya509-view',main).forEach(v=>v.remove());qq(':scope > *',main).forEach((el,i)=>{if(el.dataset.naya509Persistent==='1')return;el.hidden=false});document.body.dataset.naya509View='base'}
...
function renderProjection(name){
  ...
  if(name==='Your Intelligence'){
    const html=cards([
      '<div class="naya509-stat">'+d.length+'</div><h3>Active intelligence objects</h3><p>Your current Smart Notes are the source objects behind this view. Nothing is copied; this is a projection of the same intelligence.</p>',
      '<div class="naya509-stat">'+d.filter(x=>x.truth).length+'</div><h3>With current state</h3><p>Truth/state metadata remains attached to the underlying intelligence object.</p>',
      '<div class="naya509-stat">∞</div><h3>Next intelligent action</h3><p>Open the intelligence that matters most, understand it, apply it, then verify what changed.</p>'
    ])+'<div class="naya509-list" style="margin-top:14px">'+d.map(x=>'<div class="naya509-item"><div><b>'+x.title+'</b><span>'+x.summary.slice(0,150)+'</span></div><button class="naya509-btn" data-note="'+x.i+'">OPEN</button></div>').join('')+'</div>';
  ...
  if(name==='Your Reports'){
    const html=cards(['<h3>Intelligence over time</h3><p>Reports interpret the intelligence lifecycle: events → patterns → meaning → change → learning → result → recommendation.</p>','<h3>Current signal</h3><p>'+d.length+' canonical Smart Note objects are available to report on. Reports should interpret, not merely count.</p>','<h3>Verification</h3><p>Report claims inherit the underlying object’s provenance and truth state rather than becoming a new source of truth.</p>']);
    mountView('Your Reports','INTELLIGENCE REPORTING','Turn accumulated intelligence into patterns, decisions, results, learning, and better next actions without creating disconnected copies.',html,'blue');return;
  }
  if(name==='Intelligent Library'){
    const html='<div class="naya509-list">'+d.map(x=>'<div class="naya509-item"><div><b>'+x.title+'</b><span>'+x.truth+' · canonical intelligence</span></div><button class="naya509-btn" data-note="'+x.i+'">OPEN BOARD</button></div>').join('')+'</div>';
    mountView('Intelligent Library','CANONICAL INTELLIGENCE','The reusable body of intelligence. The Library indexes the same Smart Note objects; opening one returns you to its Intelligent Block and Smart Board presentation.',html,'yellow');return;
  }
  if(name==='Smart Share'){
    const html=cards(['<h3>Collective intelligence</h3><p>Smart Share is the shared projection of intelligence. Sharing changes visibility or contribution context; it does not create a second intelligence object.</p>','<h3>Contribution</h3><p>Useful comments, discoveries, corrections, evidence, and relationships can become new intelligence through the appropriate pathway.</p>','<h3>Human authority</h3><p>Personal intelligence remains private by default. Public contribution is an explicit human choice.</p>']);
    mountView('Smart Share','COLLECTIVE INTELLIGENCE','Where intelligence becomes voluntarily shareable and capable of creating more intelligence through other humans.',html,'pink');return;
  }
  if(name==='Smart Ledger'){
    const html='<div class="naya509-grid">'+d.map((x,i)=>'<article class="naya509-card"><h3>'+x.title+'</h3><p><strong>STATE:</strong> '+x.truth+'</p><p style="margin-top:7px"><strong>RECORD:</strong> intelligence object '+(i+1)+' · provenance preserved · verification remains distinct from interpretation.</p></article>').join('')+'</div>';
    mountView('Smart Ledger','TRUTH + PROVENANCE','The record layer: what happened, who or what acted, when, source, artifact, state change, verification, evidence, and next action.',html,'yellow');return;
  }
  if(name==='Your Connections'){
    const html='<div class="naya509-grid">'+d.map((x,i)=>'<article class="naya509-card"><h3>'+x.title+'</h3><p>Connected intelligence: Naya interpretation · related notes · reports · evidence · lists · spaces · share pathways · activity.</p><button class="naya509-btn" data-note="'+i+'" style="margin-top:12px">EXPLORE</button></article>').join('')+'</div>';
    mountView('Your Connections','RELATIONSHIP GRAPH','See how the same intelligence object relates to other intelligence, people, projects, spaces, evidence, events, decisions, results, and learning.',html,'blue');return;
  }
  if(name==='Smart Lists'){
    const groups=[['Favorites','Intelligence you intentionally marked as important.'],['Saved Intelligence','Objects you want available for later use.'],['Current Projects','Intelligence organized around active work.'],['Ideas','Potential intelligence waiting for development.'],['Learning','Lessons and discoveries worth compounding.'],['To Act On','Intelligence that has a meaningful next action.']];
    const html=cards(groups.map(g=>'<h3>'+g[0]+'</h3><p>'+g[1]+'</p><button class="naya509-btn" style="margin-top:12px">VIEW LIST</button>'));
    mountView('Smart Lists','PERSONAL ORGANIZATION','Your chosen organization layer. Lists organize the same intelligence objects without duplicating them or changing their provenance.',html);return;
  }
  if(name==='Smart Spaces'){
    const html=cards(['<h3>Mission Space</h3><p>A living environment organized around a subject, mission, problem, or opportunity.</p>','<h3>Intelligence inside</h3><p>Spaces can contain the same Smart Notes, conversations, people, contributions, evidence, learning, and next actions.</p>','<h3>Continuity</h3><p>Creating or materially changing a Space is itself consequential activity and should have an Activity/event pathway.</p>']);
    mountView('Smart Spaces','LIVING INTELLIGENCE ENVIRONMENTS','Spaces gather related intelligence and people around something that matters, while preserving the identity of every intelligence object inside.',html,'green');return;
  }
  if(name==='Smart Mail'){
    const html=cards(['<h3>Communication → intelligence</h3><p>Messages can carry intent, people, projects, possible events, actions, decisions, and discoveries.</p>','<h3>Useful consequence</h3><p>Meaningful communication can feed Smart Notes, tasks, connections, Spaces, saved intelligence, report inputs, and relationship updates.</p>','<h3>Human authority</h3><p>Mail never silently becomes an authorized action. The human remains the authority over consequential communication and commitments.</p>']);
    mountView('Smart Mail','COMMUNICATION INTELLIGENCE','Communication becomes part of the intelligence system when it creates meaningful context, action, evidence, relationships, or learning.',html,'purple');return;
  }
  showBase();
}
function bindHub(){
  ...
  buttons.forEach(el=>{
    const name=text(el);if(!NAV.includes(name))return;
    el.addEventListener('click',e=>{
      e.preventDefault();e.stopPropagation();
      buttons.forEach(x=>x.classList.toggle('active',x===el));
      if(name==='Your Intelligence')showBase();else renderProjection(name);
    },true);
  });
  const main=q('.main');if(main&&!q('.top',main))return;
  main?.addEventListener('click',e=>{const b=e.target.closest('[data-note]');if(b)gotoBlock(Number(b.dataset.note))});
}
