...
function sidebar(){const rail=q('.rail');if(!rail)return;const els=qq('.nav button,.nav a,.rail button,.rail a',rail),mails=[];els.forEach(el=>{const t=text(el);if(/^Evidence$/i.test(t)||/^Collective$/i.test(t))el.remove();else if(/^Smart Mail$/i.test(t))mails.push(el)});mails.slice(1).forEach(el=>el.remove())}
...
window.NAYA509_FEED_FINISH='2026-09-14';
})();
