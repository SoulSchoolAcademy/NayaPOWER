...
function renderItem(item){
 ...
 return '<article class="sf-card" style="--tone:'+t+'" data-sf-id="'+esc(id)+'" data-event-id="'+esc(event)+'"><div class="sf-main"><div class="sf-head"><div class="sf-jewel" aria-hidden="true">'+mark+'</div><div class="sf-head-copy"><h3 class="sf-title2">'+esc(title)+'</h3><div class="sf-meta"><span class="sf-pill">'+esc(stream.toUpperCase())+'</span><span class="sf-pill truth">'+esc(verification)+'</span><span class="sf-pill '+(stream==='collective'?'shared':'')+'">'+esc(visibility)+'</span>'+(date?'<span class="sf-pill">'+esc(date)+'</span>':'')+'</div></div></div><div class="sf-body"><span class="sf-body-label">'+(stream==='activity'?'WHAT HAPPENED':'IN A NUTSHELL')+'</span><p>'+esc(body)+'</p></div>'+(layers?'<div class="sf-layers">'+layers+'</div>':'')+'</div>'+(acts?'<div class="sf-actions">'+acts+'</div>':'')+'</article>';
}
function host(){
