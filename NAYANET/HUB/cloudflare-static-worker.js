...
async function dailyReport(url){
  ...
  const payload={ok:true,schema:'NAYANET_DAILY_REPORT_RETRIEVAL_V1',report_id:'DIR-'+date,report_type:'DAILY',report_date:date,source:{repository:REPORT_REPO,path,ref:'main',blob_sha:meta.sha||null,source_url:meta.html_url||null},provenance:{retrieved_by:'nayanet-canonical-report-gateway',retrieved_at:new Date().toISOString(),content_sha256:hash,authority:'CANONICAL_MAIN'},content};
...
export default {
  async fetch(request, env) {
    ...
    if(url.pathname.startsWith('/api/reports/daily/'))return dailyReport(url);
