...
const items:NavItem[]=[
 {name:'Intelligent',route:routes.home,icon:'◉'},
 {name:'Reports',route:routes.reports,icon:'▤'},
 {name:'Intelligent Library',route:routes.library,icon:'▦'},
 {name:'Smart Start',route:routes.today,icon:'✦'},
 {name:'Smart Ledgers',route:routes.ledger,icon:'◈'},
 {name:'Peer Connections',route:routes.connections,icon:'↔'},
...
export function AppShellV3({children}:ShellProps){
 ...
 const go=(p:string)=>{const next=normalize(p);if(next===path)return;history.pushState({},'',next);setPath(next)};const submit=()=>{const q=query.trim();if(!q)return;go(routes.feed);dispatchEvent(new CustomEvent('nayanet:search',{detail:{query:q}}))};const current=items.find(x=>x.route===path);const label=current?.name||'Intelligent';
