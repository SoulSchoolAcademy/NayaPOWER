...
function SmartFeed(){const [events,setEvents]=useState<IntelligentEvent[]>([]);const[lens,setLens]=useState<Lens>('collective');const[query,setQuery]=useState('');const[loading,setLoading]=useState(true);const[selected,setSelected]=useState<IntelligentEvent>();
 ...
 const counts=useMemo(()=>({collective:events.filter(e=>e.privacy.visibility!=='PRIVATE').length,personal:events.filter(e=>e.privacy.visibility==='PRIVATE').length,activity:events.filter(e=>Boolean(e.action.text||e.action.status||e.machine_evidence.items?.length)).length}),[events]);
 const visible=useMemo(()=>{let list=events;if(lens==='personal')list=list.filter(e=>e.privacy.visibility==='PRIVATE');if(lens==='activity')list=list.filter(e=>Boolean(e.action.text||e.action.status||e.machine_evidence.items?.length));if(lens==='collective')list=list.filter(e=>e.privacy.visibility!=='PRIVATE');const q=query.trim().toLowerCase();if(!q)return list;return list.filter(e=>[e.source.label,e.human_input.raw,e.naya_interpretation.observation||'',e.naya_interpretation.interpretation||'',e.meaning.text||'',e.whats_in_it_for_you||'',e.action.text||'',...(e.context.tags||[])].join(' ').toLowerCase().includes(q))},[events,lens,query]);
 if(selected)return <div className="feed-detail"><button className="back-button" onClick={()=>setSelected(undefined)}>← BACK TO INTELLIGENCE</button><FeedCard event={selected} index={0} lens={lens} onOpen={()=>{}}/></div>;
 ...
 <div className="feed-layout"><LeftRail lens={lens} setLens={setLens} counts={counts}/><main className="smart-feed-stage"><div className="stage-line"><div><span>{lensLabels[lens]}</span><b>{query?`${visible.length} MATCHES`:`${visible.length} INTELLIGENT OBJECTS`}</b></div><p>{lens==='activity'?'WHAT HAPPENED':lens==='personal'?'WHAT MATTERS TO YOU':'WHAT MATTERS NOW'}</p></div>{loading?<div className="feed-loading"><span className="loading-orbit"/><b>Restoring canonical intelligence…</b><p>Naya is reconnecting the human presentation layer to the intelligence source.</p></div>:visible.length?visible.map((event,i)=><FeedCard key={event.event_id} event={event} index={i} lens={lens} onOpen={()=>setSelected(event)}/>):<div className="feed-loading"><b>NO INTELLIGENCE IN THIS PROJECTION</b><p>No dead end: change the projection or search for another relationship.</p></div>}</main><RightRail event={selected}/></div></div>}
