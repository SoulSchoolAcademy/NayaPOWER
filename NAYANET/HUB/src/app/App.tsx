...
const lensTabs:LensTab[]=[{key:'personal',label:'PERSONAL',detail:'Your private intelligence and saves',icon:'◉'},{key:'activity',label:'ACTIVITY',detail:'What Naya and the system are doing',icon:'◷'},{key:'collective',label:'COLLECTIVE',detail:'Shared intelligence worth discovering',icon:'◎'}];
...
function HubHome({onExplore}:{onExplore:(event?:IntelligentEvent)=>void}){
 ...
 const open=(event?:IntelligentEvent)=>{if(event){setSelected(event);onExplore(event)}else onExplore()};
