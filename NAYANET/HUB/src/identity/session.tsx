...
export type Identity={user_id:string;session_id:string;display_name:string;smart_name:string;smart_alias:string;permissions:string[];privacy_state:string;is_authenticated:boolean;is_authenticated:boolean};
...
export function IdentityProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{
    let alive=true;
    supabase.auth.getSession().then(({data})=>{if(!alive)return;setIdentity(data.session?identityFromSession(data.session):fallback);}).catch(()=>{if(alive)setIdentity(fallback)});
