...
export type PISFeed={schema_version:string;generated_at:string;source:'supabase:nayanet_intelligence_index'|'github:smart_notes';event_count:number;events:IntelligentEvent[]};
...
function indexRowToEvent(row:IndexRow):IntelligentEvent{
  ...
  return {
    event_id:row.source_id,user_id:row.owner_id,created_at:row.created_at,updated_at:row.updated_at,
    ...
    weaver_synthesis:{summary:row.title,relationships:[]},lesson:{text:typeof metadata.lesson==='string'?metadata.lesson:undefined,retained:false},meaning:{text:typeof metadata.meaning==='string'?metadata.meaning:undefined},action:{text:typeof metadata.action==='string'?metadata.action:undefined,status},
    relationships:{event_ids:[],connection_ids:[],space_ids:[]},privacy:{visibility:privacy,consent_state:privacy},trust:{level:typeof metadata.verified_at==='string'?'verified':'recorded',evidence_ids:[row.id]},status,
...
async function loadBuildPIS():Promise<PISFeed>{
  ...
  return {...payload,source:'github:smart_notes'};
}
...
export function sortPrimaryIntelligence(events:IntelligentEvent[]):IntelligentEvent[]{return [...events].sort((a,b)=>Date.parse(b.created_at)-Date.parse(a.created_at));}
