...
(() => {
  ...
  function parseNotes(text) {
    return text.split(/={20,}/).map(x => x.trim()).filter(Boolean).map((raw, i) => {
      const lines = raw.split(/\r?\n/); const head = (lines.shift() || '').trim(); if (!/SMART NOTE/i.test(head)) return null;
      const title = (lines.shift() || '').trim(); const body = lines.join('\n').trim(); const sections = {};
      const re = /(?:^|\n)(\d+)\.\s*([^\n]+)\n([\s\S]*?)(?=\n\d+\.\s|$)/g; let m;
      while ((m = re.exec(body))) sections[Number(m[1])] = {name:m[2].trim(), text:m[3].trim()};
      return {number:i+1,title,sections};
    }).filter(Boolean);
  }
  const sectionBody = (text) => esc(text).replace(/\n/g,'<br>');
  function actions() { return `<div class="actions" role="group" aria-label="Intelligence actions">
    ...
    <span class="naya509-rating" data-c4-kind="rating" role="group" aria-label="Rate this intelligence"><span class="ratingLabel">RATE THIS INTELLIGENCE</span><span class="ratingStars">${[1,2,3,4,5].map(n=>`<button type="button" class="ratingStar" data-rating="${n}" aria-label="Rate ${n} out of 5">★</button>`).join('')}</span></span>
    <button class="action share" type="button" data-c4-kind="share">＋ SHARE INTEL</button>
  </div>`; }
  function board(note, idx) {
    const tone = tones[idx % tones.length]; const nums = Object.keys(note.sections).map(Number).sort((a,b)=>a-b); const nutshell = note.sections[1];
