-- Prevent concurrent cognition writes from allocating the same execution receipt revision.
-- The lock matches the canonical receipt namespace: authenticated user + project.
...
begin
  ...
  values(auth.uid(),p_project_id,v_state.revision,p_action,p_expected_result,p_observed_result,'SUCCESS',jsonb_build_object('event_id',v_event.event_id,'source_hash',v_event.source_hash,'created_at',v_event.created_at),coalesce(p_learning,'[]'::jsonb))
