-- Repair Smart Connect cognition receipt overload only.
-- Existing canonical nayanet_connections table and RPC model are preserved.
-- Explicit null::jsonb selects the 7-argument governed cognition recorder.
...
begin
  ...
  perform public.nayanet_record_cognition_event('NayaNET',v_event,'connection_save','Authenticated member saves a Connection from an active shared Space.','Connection persisted as active.','[]'::jsonb,null::jsonb);
...
begin
  ...
  perform public.nayanet_record_cognition_event('NayaNET',v_event,'connection_revoke','Authenticated member revokes a saved Connection.','Connection persisted as revoked.','[]'::jsonb,null::jsonb);
