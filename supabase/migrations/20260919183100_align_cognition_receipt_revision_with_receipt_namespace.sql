-- Cognition receipts share the execution-receipt namespace with other actions.
-- Serialize allocation and choose the next receipt revision from that namespace,
-- rather than assuming project cognition-state revision equals receipt revision.
...
begin
  ...
  select coalesce(max(revision),0)+1 into v_receipt_revision from public.nayanet_execution_receipts where user_id=auth.uid() and project_id=p_project_id;
  ...
  values(auth.uid(),p_project_id,v_receipt_revision,p_action,p_expected_result,p_observed_result,'SUCCESS',jsonb_build_object('event_id',v_event.event_id,'source_hash',v_event.source_hash,'created_at',v_event.created_at),coalesce(p_learning,'[]'::jsonb))
