...
begin
 ...
 bv := case when br.value ? 'verified_value' then (br.value->>'verified_value')::numeric else coalesce((br.value->>'benefit')::numeric,0)-coalesce((br.value->>'harm')::numeric,0)-coalesce((br.value->>'cost')::numeric,0)-coalesce((br.value->>'risk_adjusted_loss')::numeric,0) end;
 cv := case when cr.value ? 'verified_value' then (cr.value->>'verified_value')::numeric else coalesce((cr.value->>'benefit')::numeric,0)-coalesce((cr.value->>'harm')::numeric,0)-coalesce((cr.value->>'cost')::numeric,0)-coalesce((cr.value->>'risk_adjusted_loss')::numeric,0) end;
 improvement := cv > bv;
