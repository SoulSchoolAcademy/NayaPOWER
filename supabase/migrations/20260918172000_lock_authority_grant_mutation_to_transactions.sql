-- Authority Grant hardening V1
-- The grant record is not directly writable by authenticated clients.
-- Issuance/revocation are the only mutation paths; validator remains read-only.

revoke all on table public.nayanet_authority_grants from anon, authenticated, public;
grant select on table public.nayanet_authority_grants to authenticated;
...
returns public.nayanet_authority_grants language plpgsql security definer set search_path = '' as $$
...
revoke execute on function public.nayanet_authority_grant_immutable_guard() from public, anon, authenticated;
