-- Attack #4: bind the existing gate execution lineage into the existing cognition receipt.
-- No new authority model or persistence system is introduced.

-- Repair canonical cognition -> execution receipt lineage.
-- The existing record boundary creates the receipt after the cognition row.
-- Bind the resulting receipt ID back onto that same cognition event before returning.
-- No new persistence system or alternate event path is introduced.
...
begin
  ...
  values(
    ...
    jsonb_build_object(
      'event_id',v_event.event_id,'source_hash',v_event.source_hash,'created_at',v_event.created_at,
      'execution_lineage',coalesce(p_event->'metadata'->'execution_lineage','{}'::jsonb)
    ),
    coalesce(p_learning,'[]'::jsonb),
    nullif(p_event->'metadata'->>'authority_grant_id','')::uuid,
    nullif(p_event->'metadata'->>'authority_issuer_id','')::uuid,
    p_event->'metadata'->'authority_scope',
    p_event->'metadata'->'authority_actions',
    p_event->'metadata'->'authority_constraints',
    nullif(p_event->'metadata'->'execution_lineage'->>'governance_state',''),
    p_event->'metadata'->>'authority_source_event_id',
    nullif(p_event->'metadata'->'execution_lineage'->>'validated_at','')::timestamptz
  )
