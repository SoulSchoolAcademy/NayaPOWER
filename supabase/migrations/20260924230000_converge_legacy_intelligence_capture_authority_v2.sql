-- Close the legacy six-argument cognition RPC authority seam.
-- The canonical intelligence.capture path is the seven-argument overload with
-- explicit ExecutionAuthorization. The legacy overload remains available for
-- non-capture callers that still use its existing metadata-grant contract.
...
begin
  ...
  if p_action = 'intelligence.capture' then
    -- This overload cannot receive canonical ExecutionAuthorization.
    -- Never authorize capture from caller-supplied legacy metadata.
