-- P1 harden Smart Mail policy receipt lineage at the authorized boundary
-- The authorized transaction owns the final receipt update, so policy identity,
-- experiment case, input hash and decision hash cannot be lost across overloaded
-- transaction functions or stale RPC resolution.
  ...
  if coalesce(v_result->>'status','')='CREATED' then
    update public.nayanet_execution_receipts
      set authority_grant_id=(v_authority->>'grant_id')::uuid,
          ...
          policy_version=(
            select pv.version from public.nayanet_policy_versions pv where pv.id=p_policy_id
          ),
          policy_key=(
            select pv.policy_key from public.nayanet_policy_versions pv where pv.id=p_policy_id
          ),
          policy_parent_id=(
            select pv.parent_policy_id from public.nayanet_policy_versions pv where pv.id=p_policy_id
          ),
...
grant execute on function public.nayanet_send_smart_mail_authorized(
  uuid,uuid,text,text,text,text,text,uuid,text,text,text,uuid
