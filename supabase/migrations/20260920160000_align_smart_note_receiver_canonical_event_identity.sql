-- Align the canonical Smart Note receiver identity.
-- Edge Function supplies one UUID in p_human_note.event_id.
-- This transaction boundary preserves that UUID as smart_note_events.id.
...
begin
  ...
  if v_subject is null then
    v_subject := left(regexp_replace(coalesce(p_human_note->>'text',p_human_note->>'content','Smart Note'),'\s+',' ','g'),160);
  end if;
  ...
  values(
    ...
    jsonb_build_object('idempotency_key',p_idempotency_key,'source',coalesce(p_machine_note->>'source','naya_conversation'),'captured_at',v_now),
  ...
  values(
    p_idempotency_key,auth.uid(),'completed',p_human_note,p_naya_note,p_machine_note,p_intelligent_feed,
    p_intelligent_block,
    p_evidence || jsonb_build_object('event_id',v_event_id,'verified_at',v_now),
    p_hub_state || jsonb_build_object('event_id',v_event_id,'last_intelligence_event_at',v_now)
  )
