-- Wire Authority Grant V1 into the canonical intelligence_commit capture receipt boundary.
-- Existing record call shape is preserved. Only the intelligence.capture action requires
-- an authority grant; the validated grant is persisted onto the execution receipt.
...
-- Preserve the established execute boundary on the existing function signature.
