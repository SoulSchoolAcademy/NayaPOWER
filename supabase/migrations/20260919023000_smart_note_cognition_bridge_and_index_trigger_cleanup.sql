-- Minimum Smart Note -> Cognition bridge + duplicate index-trigger cleanup.
-- Smart Note remains the canonical domain transaction; Cognition is the generalized event identity.
-- The bridge is idempotent on (user_id, project_id, event_id) and preserves Smart Note identity/provenance.
...
as $$
declare
  v_project_id text := coalesce(nullif(trim(new.source_context->>'project_id'), ''), 'SMART_NOTE');
...
drop trigger if exists trg_smart_note_events_to_intelligence_index on public.smart_note_events;