-- Keep owner and direct-join membership provenance explicit in the current-state row.
create or replace function public.nayanet_space_owner_membership()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  insert into public.nayanet_space_members(space_id,member_id,role,status,source_type,source_id)
  values (new.id,new.owner_member_id,'owner','active','space_owner',new.id)
  on conflict (space_id,member_id)
  do update set role='owner',status='active',left_at=null,source_type='space_owner',source_id=new.id,updated_at=now();
  return new;
end;
$$;

update public.nayanet_space_members
set source_type='space_owner', source_id=space_id
where role='owner';
...
begin
  ...
  if v_row.id is null then
    ...
    values(p_space_id,v_member,'member','active',now(),null,'direct_join',p_space_id)
    returning * into v_row;
  else
    update public.nayanet_space_members set status='active',joined_at=now(),left_at=null,source_type='direct_join',source_id=p_space_id,updated_at=now()
  ...
  perform public.nayanet_record_cognition_event('NayaNET',v_event,'space_join','Authenticated member becomes an active member of the shared Space.','Membership row persisted as active.','[]'::jsonb);
