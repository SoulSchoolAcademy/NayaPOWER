-- NayaNET persistent cognition status contract repair.
-- Mirrors the production migration applied after runtime verification found
-- mismatched CHECK constraints in the canonical cognition RPC path.
...
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  update public.nayanet_project_cognition_state set revision=revision+1,state=coalesce(state,'{}'::jsonb)||jsonb_build_object('successor',p_handoff),updated_at=now(),status='READY' where user_id=auth.uid() and project_id=p_project_id returning * into v;
  ...
  values(auth.uid(),p_project_id,v.revision,'successor_handoff','SUCCESS',jsonb_build_object('handoff_id',p_handoff->>'handoff_id','last_event_id',p_handoff->>'last_event_id'),jsonb_build_array('successor state persisted'));
  return v;
end;
$$;
