...
begin
  if auth.uid() is null then
    if auth.role() <> 'service_role'
       or p_action <> 'github_webhook_received'
       ...
       or coalesce(p_execution_authorization->>'repository','') = '' then
      raise exception 'AUTH_REQUIRED';
    end if;
    v_actor_id := (p_execution_authorization->>'actor_id')::uuid;
    v_is_github_webhook := true;
    if public.nayanet_resolve_github_webhook_owner(
      (p_execution_authorization->>'installation_id')::bigint,
      p_execution_authorization->>'repository'
    ) <> v_actor_id then
      raise exception 'GITHUB_WEBHOOK_OWNER_BINDING_MISMATCH';
    end if;
  else
    v_actor_id := auth.uid();
  end if;
  ...
  values(v_actor_id,p_project_id,0,jsonb_build_object('last_event_id',v_event.event_id,'last_event_at',v_event.created_at,'successor',null),'READY') on conflict (user_id,project_id) do nothing;
  update public.nayanet_project_cognition_state set revision=revision+1,state=coalesce(state,'{}'::jsonb)||jsonb_build_object('last_event_id',v_event.event_id,'last_event_at',v_event.created_at),status='IN_PROGRESS',updated_at=now()
   where user_id=v_actor_id and project_id=p_project_id returning * into v_state;
