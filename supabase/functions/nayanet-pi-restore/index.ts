...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    return json({ok:response.ok,edge_function:"nayanet-pi-restore",action:ACTION,delegated_to:"nayanet-compound-intelligence",result},response.status);
  }catch(e){
    return json({ok:false,edge_function:"nayanet-pi-restore",action:ACTION,error:String((e as Error)?.message||e)},502);
  }
});