...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const machine={event_id:eventId,schema:"nayanet.smart_note.machine.v2",occurred_at:now,source:body?.source||"naya_conversation",type:human.type||"insight",actor:user.id,human_note_id:canonicalHuman.id||null,normalized_text:human.text||human.content||"",idempotency_key:idempotencyKey};
  ...
  const {data,error}=await supabase.rpc("v7_create_smart_note",{p_idempotency_key:idempotencyKey,p_user_id:user.id,p_human_note:canonicalHuman,p_naya_note:canonicalNaya,p_machine_note:machine,p_intelligent_feed:feed,p_intelligent_block:block,p_evidence:evidence,p_hub_state:hubState,p_subject:typeof body?.subject==="string"?body.subject:null});
