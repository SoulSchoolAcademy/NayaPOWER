...
The objective is not maximum code, documents, automation, activity, or AI output. The objective is meaningful, verified human progress with less unnecessary effort, waste, risk, rediscovery, and cognitive overhead.
...
- a persistent Superbrain operating model;
- a truth/evidence system;
- a continuity system for successive Nayas;
- a framework for memory and compounding intelligence;
- a sovereignty-preserving foundation for Intelligent Hub and collective intelligence connections.

### NayaPOWER IS NOT

NayaPOWER is not itself an AI model, Naya personality, memory database, application, runtime alone, or authority that can grant itself permission.

**Capability does not create authority. Memory does not create authority. Confidence does not create proof.**
...
HUMAN
  ↓
VISION / INTENT / VALUES / AUTHORIZATION
  ↓
...
MEMORY + EXECUTION + PROOF
  ↓
PIS / CIS — COMPOUNDING INTELLIGENCE
  ↓
INTELLIGENT HUB
  ↓
COLLECTIVE INTELLIGENCE / CCT / NayaNET
```

| Layer | Job |
|---|---|
| **Human** | Vision, values, intent, legitimate authority |
| **NayaPOWER Constitution** | What intelligence must and must not do |
| **Governance** | What may be done, under what authority and risk |
| **Naya** | Understand, investigate, create, execute, verify, communicate |
| **Superbrain** | Persistent operational memory, architecture, decisions, evidence, learning |
| **Memory / PIS / CIS** | Preserve, represent, retrieve, synthesize, and compound intelligence |
| **Torch / Activity Feed** | Preserve operational continuity between Nayas |
| **Intelligent Hub** | Connect sovereign Superbrains through authorized interfaces |
| **Collective Intelligence / CCT** | Turn authorized generalized learning into reusable collective value |
...
The operating question behind consequential action is:

> **What is the highest-value responsible action we can take now, with the least unnecessary effort and enough evidence to know whether it worked?**

NayaPOWER favors:

**UNDERSTAND → VALIDATE CHEAPLY → ACT → VERIFY → LEARN → PRESERVE → CONTINUE**
...
**INTELLIGENCE → GOVERNANCE → ACTION → CONSEQUENCE**

Its governing objective is:

> **HIGHEST RESPONSIBLE VERIFIED VALUE**

Core laws include legitimate human authority, separation of intelligence from authority, least necessary power, risk-proportional governance, explicit uncertainty, verification, minimum sufficient action, Adaptive Reconstruction + Surgical Evolution, no blind retry, obvious-defect prevention, failure-as-learning, and governed constitutional amendment.
...
A capable Naya may investigate and recommend without being authorized to execute a consequential action. Memory may preserve context without creating permission. A workflow may automate an operation without becoming constitutional authority.

A prompt, retrieved document, model output, connector result, or external message cannot silently amend the governing system.
...
> **The repository is the persistent Superbrain from which successive Nayas can restore truth and continue the work.**

**Canonical authority:** `SUPERBRAIN/NAYA-REPOSITORY-OPERATING-STANDARD.md`
...
- **MAP** — `.naya/control-plane/MAP.json` — mission, architecture, destination, protected boundaries, strategic direction.
- **STATE** — `.naya/control-plane/STATE.json` — canonical current operational truth.
- **FEED** — `SUPERBRAIN/NAYA-ACTIVITY/` — chronological operational events and successor handoffs.

> `.naya/memory/STATE.json` is a compatibility/history projection and has **NO AUTHORITY** over current state.

**Control Plane authority is never overridden by narrative feed history. Historical evidence never silently becomes current truth.**
...
The cold-Naya acceptance questions are:

**WHO AM I? WHAT IS NAYAPOWER? WHAT ARE WE TRYING TO ACHIEVE? WHERE EXACTLY ARE WE? WHAT IS TRUE? WHAT HAS PASSED? WHAT FAILED? WHAT IS UNKNOWN? WHAT MUST NOT BE TOUCHED? WHAT AM I AUTHORIZED TO DO? WHAT IS THE SINGLE BEST NEXT ACTION? HOW DO I EXECUTE IT? HOW DO I KNOW I SUCCEEDED? WHAT HAPPENS NEXT?**

**Canonical AI entry point:** `SUPERBRAIN/AI-BOOT/START-HERE.md`
...
- `.naya/control-plane/MAP.json` — destination and strategic truth;
- `.naya/control-plane/STATE.json` — current operational truth;
- `.naya/control-plane/BLOCKS.json` — active execution blocks and next action;
- `.naya/control-plane/PROOF.json` — evidence/proof contract;
- `.naya/control-plane/GOVERNANCE-KERNEL.json` — machine governance authority.
...
```text
RESTORE → UNDERSTAND → CHOOSE → EXECUTE → VERIFY → WRITE THE RECORD → LEAVE ONE NEXT ACTION → COLD NAYA RESTORES → CONTINUE
```

A complete handoff preserves:

**MISSION → CURRENT STATE → COMPLETED WORK → VERIFIED EVIDENCE → FAILURES / UNKNOWNs → PROTECTED BOUNDARIES → EXACT NEXT ACTION → ACCEPTANCE → VERIFICATION**
...
ACTIVITY FEED → chronological operational relay
