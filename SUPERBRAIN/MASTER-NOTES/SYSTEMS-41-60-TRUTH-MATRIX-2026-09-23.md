...
| 54 | Current-Truth Compiler | IMPLEMENTED — NOT VERIFIED | PR #563: generator, smoke test, CI workflow, contract doc; live HEAD is resolved at generation time | Execute workflow and inspect generated artifact; independently verify freshness/conflict detection | 55 |
| 55 | Cold-Naya Benchmark | BLOCKED ON 54 | Canonical boot chain and cold-Naya acceptance rules already exist | Need measured benchmark: reconstruction accuracy, current-state accuracy, authority/proof/unknown recognition, next-action accuracy, archaeology count | 54, 60 |
...
| 57 | Hub as Projection | PARTIAL / STRONGLY CONTRACTED | Hub Read-First + visual/structural contract explicitly state one brain / many projections; canonical Hub path is protected | Need source→intelligence→projection→action proof showing Hub does not become a second store/brain | 41, 43, 45, 58 |
...
| 59 | Why Naya Explanation Layer | MISSING/PARTIAL | Retrieval explanation is explicitly absent in Coda 1's 1–20 truth map; current Hub contracts describe evidence/provenance but not a verified explanation surface | Need concise evidence-oriented reason output: prior intelligence + current evidence + constraints + authority + uncertainty, with no chain-of-thought | 11, 41, 57 |
