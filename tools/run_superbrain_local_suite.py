...
CORE=[ROOT/"tools/qa_naya_context_boot.py",ROOT/"tools/qa_superbrain_continuity.py",ROOT/".naya/runtime/restore_context.py",ROOT/"SUPERBRAIN/test_naya_power_excellence.py",ROOT/"SUPERBRAIN/test_naya_power_decision_calculus.py",ROOT/"tools/test_superbrain_a_b_c_compounding.py",ROOT/"tools/test_superbrain_adversarial.py"]
...
def selected_commands():
 ...
 for path in CORE:
  ...
  commands.append([sys.executable,str(path.relative_to(ROOT)),"restore","--pretty"] if path.name=="restore_context.py" else [sys.executable,str(path.relative_to(ROOT))])
...
def main()->int:
 ...
 receipt={"schema":"naya-power-local-superbrain-suite/v5","started_at":started.isoformat(),"finished_at":datetime.now(timezone.utc).isoformat(),"observed_head":head,"clean_worktree_before":clean_before,"github_actions_used":False,"selected_check_count":len(commands),"excluded_checks":EXCLUDED_LEGACY_CONTRACTS,"mandatory_layers":["EXCELLENCE_BY_DEFAULT","DECISION_CALCULUS","VERIFICATION","SMART_NOTE_PROMOTION","A_TO_B_TO_C_COMPOUNDING"],"commands":results,"overall":"PASS" if overall==0 else "FAIL"}
