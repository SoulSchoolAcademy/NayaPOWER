...
def normalize(text:str)->str:
    return " ".join(t for t in re.findall(r"[a-z0-9]+",text.lower()) if t not in STOPWORDS and len(t)>2)

def fingerprint(e:dict[str,Any])->str:
    basis="|".join(normalize(str(e.get(k,""))) for k in ("project","lesson","root_cause","recommendation"))
...
def classify(e):
    explicit=[x for x in e.get("candidate_homes",[]) if x in HOMES]
    if explicit:homes=list(dict.fromkeys(explicit))
    else:
...
def write_note(e,target,heading):
    target.mkdir(parents=True,exist_ok=True);p=target/f"{e['event_id']}.md"
...
def prior_index(events):return {"by_id":{e["event_id"]:e for _,e in events},"normalized":{e["event_id"]:normalize(str(e.get("lesson",""))) for _,e in events}}
...
def write_feed(e,homes,decision,duplicate,status):
    FEED_DIR.mkdir(parents=True,exist_ok=True);p=FEED_DIR/f"{e['event_id']}.md";dup=f"`{duplicate}`" if duplicate else "None"
    text=f"""# 🔱 Intelligence Feed — {e.get('title',e['event_id'])}\n\n**Event:** `{e['event_id']}`  \n**Timestamp:** {e['timestamp']}  \n**Project:** {e['project']}  \n**Promotion:** `{status}`  \n**Decision:** `{decision}`  \n**Durable homes:** {', '.join(f'`{h}`' for h in homes)}  \n**Duplicate / related prior event:** {dup}\n\n## LESSON\n{e['lesson']}\n\n## VALUE / IMPACT\n{e.get('value','')}\n\n## SOURCE\n"""+"\n".join(f"- `{x}`" for x in e.get("source",[]))+f"""\n\n## EVIDENCE STATE\n`{e['evidence_state']}`\n\n## SUCCESSOR INSTRUCTION\n{e.get('successor_instruction','')}\n\n## PROMOTION\nThis entry is append-oriented intelligence. It does not override canonical governance or verified project truth.\n"""
    if not p.exists() or p.read_text(encoding="utf-8")!=text:p.write_text(text,encoding="utf-8")
    return display_path(p)

def update_hub(summary):
    if not HUB_PATH.exists():return
    a,b="<!-- PROMOTION-ENGINE-V1:START -->","<!-- PROMOTION-ENGINE-V1:END -->";existing=HUB_PATH.read_text(encoding="utf-8")
    block=[a,"","## 🔱 PROMOTION ENGINE V1 — CURRENT OPERATIONAL STATE","","The Promotion Engine processes canonical Intelligence Events into durable, auditable intelligence. This managed section is generated from the current event corpus; it does not replace canonical synthesis.",""]
    ...
    if a in existing and b in existing:updated=existing.split(a,1)[0]+new+existing.split(b,1)[1]
...
def main():
    ...
    index=prior_index(events);receipts=[]
    for p,e in events:
